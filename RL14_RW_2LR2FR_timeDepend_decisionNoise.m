% RL 14: Rescorla-Wagner (RW) 2 learning rates (LR) + 2 forgetting rates (FR) + time-dependent decision noise model 
% 
% Simulation & plotting script for the three-armed bandit (TAB) task 

% Reference: https://pmc.ncbi.nlm.nih.gov/articles/PMC7549782/
%% Set up
...
% RW 2LR + 2FR + time-depend decision noise model parameters
% 1) learning rate for wins (range between 0 and 1)
params.alpha_win = 0.5;
% 2) learning rate for losses (range between 0 and 1)
params.alpha_loss = 0.7;
% 3) forgetting rate for wins (range between 0 and 1)
params.psi_win = 0.25;
% 4) forgetting rate for losses (range between 0 and 1)
params.psi_loss = 0.15;
% 5) information bonus (any real value)
params.gamma = 0.01;
% 6) upward shift in overall inverse temperature (range > 0)
params.beta_shift = 5;
% 7) steepness of change in inverse temperature over time (any real value)
params.beta_slope = 0.1;
...
for t = 1:T
    ...
    % Compute beta values. 
    beta(t) = params.beta_shift*((t/10).^params.beta_slope);
    ...
    if sim_rewards(1, t) == 1
        expected_reward_value(choice_at_t, t+1) = expected_reward_value(choice_at_t, t) + params.alpha_win*prediction_error;
    elseif  sim_rewards(1, t) == 0
        expected_reward_value(choice_at_t, t+1) = expected_reward_value(choice_at_t, t) + params.alpha_loss*prediction_error;
    end
    
    % Forgetting for unchosen options.
    unchose_opt_ls = find([1 2 3] ~= choice_at_t);
    if sim_rewards(1, t) == 1
        expected_reward_value(unchose_opt_ls, t+1) = (1-params.psi_win)*(expected_reward_value(unchose_opt_ls, t));
    elseif  sim_rewards(1, t) == 0
        expected_reward_value(unchose_opt_ls, t+1) = (1-params.psi_loss)*(expected_reward_value(unchose_opt_ls, t));
    end
    
    % Store the value of prediction error. 
    prediction_error_sequence(t) = prediction_error;
    
    % Trims final value in expected reward value (is an extra value beyond
    % trials).
