% AInf 11: 2 learning rates (LR) + 2 forgetting rates (FR) + error-dependent decision noise model 
% 
% Simulation & plotting script in the three-armed bandit (TAB) task 
...
% AInf 2LR + 2FR + error-dependent decision noise model parameters
...
params.beta_0 = 6;
% 7) inverse temperature update rate (range > 0)
params.alpha_d = 2;
...
for t = 1:T
    ...
    % Compute variational free energy defined by surprise when comparing observed and expected
    ...
    % Update inverse temperature on the next trial.
    ...
    if sim_rewards(1,t) == 1
        v{t+1} = (v{t} - v_0)*(1-params.psi_win) + v_0; 
    elseif sim_rewards(1,t) == 2
        v{t+1} = (v{t} - v_0)*(1-params.psi_loss) + v_0; 
    end
