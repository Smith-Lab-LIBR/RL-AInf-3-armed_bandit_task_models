% AInf 3: 2 learning rates (LR) benchmark prior model (see https://pubmed.ncbi.nlm.nih.gov/32801113/)
% 
...
% AInf 2LR benchmark prior model parameters
...
% 3) information insensitivity (initial baseline confidence; range > 0)
params.v0 = 0.25;
% 4) reward sensitivity (range > 0)
params.cr = 4;
...
v_0 = [params.v0 params.v0 params.v0;
       params.v0 params.v0 params.v0];
...
for t = 1:T
    ...
    if sim_rewards(1,t) == 1 
        v{t+1} = v{t} + params.alpha_win*(B_pi(:,choice_at_t)*outcome_vector(:,t)')';
    elseif sim_rewards(1,t) == 2
        v{t+1} = v{t} + params.alpha_loss*(B_pi(:,choice_at_t)*outcome_vector(:,t)')';
    end
